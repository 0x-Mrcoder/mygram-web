...
Route::middleware('throttle:limit-check')->group(function () {
    ...
    */
    ...
    Route::middleware(['auth', 'validity'])->group(function () {
        // ✅ CREATE TOPUP ORDER (POST)
Route::post('/home/create_topup_order', 
    [UserController::class, 'createTopupOrder']
)->name('create_topup_order');

// ✅ GENERATE VIRTUAL ACCOUNT (POST)
Route::post('/user/generate-virtual-account', [UserController::class, 'generateVirtualAccount'])->name('user.generate_virtual_account');

// ✅ UPDATE NAME (POST)
Route::post('/user/update-name', [UserController::class, 'updateName'])->name('user.update_name');
Route::get('/user/update-name', function() { return redirect()->back(); }); // Fallback for accidental GET access

// ✅ VIRTUAL ACCOUNT PAYMENT UI
Route::get('/user/payment/virtual-account/{id}', [UserController::class, 'virtualAccountPayment'])->name('user.virtual_account_payment');
Route::get('/user/payment/check-status/{id}', [UserController::class, 'checkPaymentStatus'])->name('user.check_payment_status');



// ✅ MANUAL PAYMENT PAGE (GET)
Route::get('/home/manual_payment/{id}', 
    [UserController::class, 'manualPaymentPage']
)->name('manual_payment_page');


Route::post('/home/confirm_payment/{id}', [UserController::class, 'confirmPayment'])
    ->name('confirm_payment');
        ...
        Route::get('user/payment/callback', [UserController::class, 'paymentCallback'])->name('payment.callback');
        ...
        Route::post('/validate-bank-account', [UserController::class, 'validate_bank_account'])->name('validate.bank.account');
...
Route::view('/faq', 'app.main.faq')->name('faq');
Route::view('/charity', 'app.main.charity')->name('charity');
...
// ✅ PAYRANT WEBHOOK (Public)
Route::post('/webhook/payrant', [UserController::class, 'payrantWebhook'])->name('webhook.payrant');
